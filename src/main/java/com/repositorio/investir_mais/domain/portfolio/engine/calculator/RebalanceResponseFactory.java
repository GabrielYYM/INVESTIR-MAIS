...
public class RebalanceResponseFactory {
    public RebalanceResponseDTO build(Portfolio portfolio,
        ...
        for (AssetCategory category : portfolio.getCategories()) {
            BigDecimal redistributedTargetPercentage = redistributedCategoryTargets.getOrDefault(category.getId(),
                    BigDecimal.ZERO);
            BigDecimal currentCategoryValue = category.getAssets().stream().map(Asset::getCurrentPositionValue)
                    .reduce(BigDecimal.ZERO, BigDecimal::add);
            BigDecimal categoryCurrentPercentage = totalCurrentValue.compareTo(BigDecimal.ZERO) > 0
                    ? currentCategoryValue.multiply(new BigDecimal("100")).divide(totalCurrentValue, 2,
